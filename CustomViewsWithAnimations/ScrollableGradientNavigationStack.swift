//
//  ScrollableGradientNavigationStack.swift
...
public struct ScrollableGradientNavigationStack<Content: View>: View {
  ...
  private func calculateEndPointForScrollPosition(scrollPosition: Double)
    -> Double
  {
    let absoluteScrollPosition = abs(scrollPosition)
    let endPoint =
      heightPercentage - (heightPercentage / maxHeight) * absoluteScrollPosition

    return endPoint.clamped(to: 0...heightPercentage)
  }

  private func checkScrollPositionAndGetEndPoint(scrollPosition: Double)
    -> Double
  {
  ...
  public init(
    heightPercentage: Double, maxHeight: Double, minHeight: Double,
...
#Preview {
  ScrollableGradientNavigationStack(
    heightPercentage: 0.4, maxHeight: 200, minHeight: 0, startColor: Color.red,
